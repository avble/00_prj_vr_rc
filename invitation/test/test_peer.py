...
def test_view_index(client, auth):
    ...
    response = client.get("/")
    assert "invitation" in response.data.decode('utf-8')
...
def test_view_delete(client):
    response = client.post('/peer/delete/123465')
    data = response.data.decode('utf-8')
    assert response.status_code == 302 # Status code
    assert 'redirect' in data

    response = client.get('/peer/get/123465')
...
def test_api_get_1(client):
    response = client.get("/peer/get/123456")
    data = response.data.decode('utf-8')
    assert response.status_code == 200
    
def test_api_register_1(client):
    response = client.post("/peer/register")
    assert response.status_code == 200
    code = response.data.decode('utf-8')
    assert len(code) == 6

    response = client.get("/peer/get/%s" % (code))
    data = response.data.decode('utf-8')
    assert response.status_code == 200
